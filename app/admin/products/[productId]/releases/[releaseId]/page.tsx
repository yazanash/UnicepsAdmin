...
import { useState, useEffect } from "react"; // أضفنا useEffect فقط
...
export default function NewReleasePage() {
  ...
  useEffect(() => {
    api.get(`/Release/GetRelease/${releaseId}`).then((res) => {
      const data = res.data;
  ...
  const handleSave = async () => {
    ...
    // إذا تم اختيار ملف جديد نرفعه، وإلا نعتمد الـ URL الموجود
    if (file) {
      finalDownloadUrl = await uploadFileInChunks(file);
    }
    ...
    await api.put(`/Release/${releaseId}`, payload);
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold">
        {`Edit Release for Product ${productId}`}
      </h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          <Label>File (Optional - only to replace)</Label>
          <Input
            type="file"
        ...
        {progress > 0 && progress < 100 && (
          <div className="w-full bg-gray-200 rounded col-span-3">
            <div
              className="bg-blue-500 text-white text-xs leading-none py-1 text-center rounded transition-all"
      ...
      <Button onClick={handleSave}>Update Release</Button>
